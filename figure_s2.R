##Figure S2
##s2.a - map of scores
lims = round(c(min(sf_county_final$PC4_scores,na.rm=T), max(sf_county_final$PC4_scores,na.rm=T)),digits = -1)
aa = ggplot() +
  geom_sf(data = sf_county_final, aes(fill=PC4_scores), lwd=0)+
  ...
  guides(fill = guide_colorbar(title="PC4 Score", direction = "vertical", title.position = "left",
...
##s2.b - yield quintile movmenets
yld_diff_1_l =  quintile_ylds[[4]]
yld_diff_1_l$PC4_scores_quint = as.factor(yld_diff_1_l$PC4_scores_quint)
bb = ggplot()+
  geom_hline(yintercept = 0, col="black", size = 0.25)+
  geom_line(data = yld_diff_1_l, 
            aes(x = Year, y = Yield, group = PC4_scores_quint),
            size = .65, color="black")+
  geom_line(data = yld_diff_1_l, 
            aes(x = Year, y = Yield, group = PC4_scores_quint, col=PC4_scores_quint),
            size = .6)+
...
##s2.c - normalized PC vector 
mean_spei_norm = (mean_spei - mean(mean_spei, na.rm = T)) / sd(mean_spei, na.rm=T) #normalize
ev4 = -ev[,4]
ev4_norm = (ev4 - mean(ev4, na.rm = T)) / sd(ev4, na.rm=T) #normalize
df_plot2c = data.frame(1980:2015, ev4_norm, mean_spei_norm)
names(df_plot2c) = c("Year", "PC4", "SPEI(t)")
...
cc = ggplot()+
  ...
  annotate("text",x=1997,y=2,label=expression(italic("1993 flood")), size = 2.75) 

##s2.d - map of SPEI 
spei1993 = as(spei_mask[[14]],"SpatialPixelsDataFrame")
spei1993_df = as.data.frame(spei1993)
lims2 = round(c(minValue(spei_mask[[14]]),maxValue(spei_mask[[14]])), digits=2)
dd = ggplot() +
  geom_raster(data=spei1993_df, aes(x=x, y=y, fill=X1993_gs_spei))+
...
#all together 
pdf("figure_s2.pdf", width = 7.48, height = 4)
